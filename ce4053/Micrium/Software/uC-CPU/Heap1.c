...
void buildMinHeap(minHeap *hp, int *arr, int size) {
  ...
  for(i = 0; i < size; i++) {
    if(hp->size) {
      hp->elem = realloc(hp->elem, (hp->size + 1) * sizeof(node1)) ;
    } else {
      hp->elem = malloc(sizeof(node1)) ;
    }
    node1 nd ;
    nd.data = arr[i] ;
    hp->elem[(hp->size)++] = nd ;
...
void insertHeapNode(minHeap *hp, int data, OS_TCB *p_tcb) {
  ...
  if(hp->size) {
    while(i<= size){     
      if(hp->elem[i].data == data)
      {
        OS_TCB *tempTCB = hp->elem[i].p_tcb->sameDeadline;
        while(tempTCB != NULL){
          tempTCB = tempTCB->sameDeadline;
        }
        tempTCB->sameDeadline = p_tcb;
        return;
      }
      i++;
    }
...
void deleteHeapNode(minHeap *hp) {
  if(hp->size) {
    ...
    hp->elem = realloc(hp->elem, hp->size * sizeof(node1)) ;
    heapify(hp, 0) ;
  } else {
    printf("\nMin Heap is empty!\n") ;
    free(hp->elem) ;
