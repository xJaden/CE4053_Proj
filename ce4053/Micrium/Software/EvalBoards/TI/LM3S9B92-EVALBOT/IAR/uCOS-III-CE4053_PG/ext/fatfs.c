...
//
//*****************************************************************************
CPU_INT32S CmdLineProcess(char *pcCmdLine)
{
    static char *argv[CMDLINE_MAX_ARGS + 1];
    char *pcChar;
    CPU_INT32S argc;
    CPU_INT32S bFindArg = 1;
...
/*
This file is made in CHiPES. It contains some functions from TI as it is, some 
handwritten and some TI functions modified by me.

This file contains functions that starts SPI mode of controller to make use of 
micro SD card slot. It also initialize the SD card.
It also incorporates FAT file system to work on txt files saved in SD card.
*/
...
//*****************************************************************************
//
// This function seperate the tag value from the line that it recieves and save
// it to provided variable.
//
//*****************************************************************************
void GetTagValue(char *cLine, char* cTag)
{
    unsigned short int usItrVar = 1;

    while( usItrVar < strlen(cLine) && cLine[usItrVar] != '>' )
    {
        //
        // Copy the data after '<', until we get '>'.
        //
        cTag[usItrVar-1] = cLine[usItrVar];
        usItrVar++;
    }

    //
    // Null terminate the Tag that was read to make it a null
    // terminated string.
    //
    cTag[usItrVar-1] = 0;
}
...
// This should only be used on text files.  If it is used on a binary file, then
// a bunch of garbage is likely to recieved.
//
//*****************************************************************************
CPU_INT32S Cmd_cat(int argc, char *argv[])
{
    FRESULT fresult;
    ...
    //
    // Enter a loop to repeatedly read data from the file, until the desired tag
    // is matched or end of the file is reached.
    // Once the tag is matched, it will save values related to tag for future
    // reference and will return 0, otherwise any positive number;
    //
    do
    {
        //
        ...
        //
        fresult = f_read(&g_sFileObject, g_cTmpBuf, FILE_LINE_SIZE,
                         &usBytesRead);

        //
        // If there was an error reading, then return the error to the user.
        //
        if(fresult != FR_OK)
        {
            return(fresult);
        ...
        //
        // Seperate the tag value from the line which we have just read.
        //
        GetTagValue(g_cTmpBuf, cTag);
    ...
    while(usBytesRead == FILE_LINE_SIZE && strcmp(cTag, argv[2]));
    ...
    //
    // If there was some problem closing the file, then return an error.
    //
    if(fresult != FR_OK)
    {
        return(fresult);
    ...
    //
    // If no Tag is matched, then return error.
    //
    if( usBytesRead != FILE_LINE_SIZE )
    {
        return(1);
    }

    //
    // Tag is matched. Required result is saved into g_cTmpBuf. Returns success.
    //
    return(0);
}
...
//*****************************************************************************
//
// This function implements the "app" command.  It writes the contents to a
// file.  This should only be used on text files.  If it is used on a binary
// file, then a bunch of garbage is likely to printed on the file.
//
//*****************************************************************************
CPU_INT32S Cmd_app(int argc, char *argv[])
{
    FRESULT fresult;
    unsigned short usVarLen;
    const char* ccNewLine = "\r\n";
    char cTmpBuf[PATH_BUF_SIZE];
    DWORD fSize;
    ...
    //
    fresult = f_open(&g_sFileObject, g_cTmpBuf, FA_OPEN_ALWAYS|FA_WRITE|FA_READ);
    //fresult = f_open(&g_sFileObject, g_cTmpBuf, FA_OPEN_ALWAYS|FA_WRITE|FA_READ|FA_CREATE_NEW);
    ...
    //
    fSize = f_size(&g_sFileObject);
    if(fSize >=2)
        fSize -= 2;
    fresult = f_lseek(&g_sFileObject, (DWORD)fSize);

    //
    // If there was some problem in moving cursor, then return an error.
    //
    if(fresult != FR_OK)
    {
        return(fresult);
    ...
    // Writting Tag...
    //
    sprintf(cTmpBuf, "<%s> ", argv[2]);
    sprintf(g_cTmpBuf, "%-*s", TAG_SIZE+1, cTmpBuf);
    fresult = f_write(&g_sFileObject, g_cTmpBuf, strlen(g_cTmpBuf), &usVarLen);

    //
    // Writting Data1...
    //
    sprintf(cTmpBuf, "<%s> ", argv[3]);
    sprintf(g_cTmpBuf, "%-*s", DATA1_SIZE+1, cTmpBuf);
    fresult = f_write(&g_sFileObject, g_cTmpBuf, strlen(g_cTmpBuf), &usVarLen);

    //
    // Writting Data2...
    //
    sprintf(cTmpBuf, "<%s>", argv[4]);
    sprintf(g_cTmpBuf, "%-*s", DATA2_SIZE, cTmpBuf);
    fresult = f_write(&g_sFileObject, g_cTmpBuf, strlen(g_cTmpBuf), &usVarLen);

    //
    // Write New Line command at the END
    //
    fresult = f_write(&g_sFileObject, ccNewLine, sizeof(ccNewLine),
                      &usVarLen);
...
/* Create a new file                                                          */
/******************************************************************************/
CPU_INT32S Cmd_new(int argc, char *argv[])
{
    FRESULT fresult;
    //char cTmpBuf[PATH_BUF_SIZE];
    ...
    /* Open the file for writting */
    fresult = f_open(&g_sFileObject, g_cTmpBuf, FA_CREATE_ALWAYS);
...
{
    { "new",    Cmd_new,      "  : Create new text file" },
    { "cat",    Cmd_cat,      "  : Show contents of a text file" },
    { "app",    Cmd_app,      "  : Add contents to a text file" },
    { 0, 0, 0 }
};

//
// This function writes data to file. Writting format is as follows:
// Tag    PreviousState   Amount
// If Tag is already present in the File, then its Field's values are modified.
// It returns 0, if it is successful.
//
CPU_INT32S WriteToFile(char* FileName, char* Tag, char* Data1, char* Data2)
{
    int nStatus;
    ...
    //
    // Copying the desired command
    //
    sprintf(g_cCmdBuf, "app %s %s %s %s", FileName, Tag, Data1, Data2);

    //
    // Pass the line to the command processor.
...
//
// This function reads data from file. It searches in file, for given Tag which
// is saved in g_LogDataFormat* Data, and save all values related to this tag in
// same struct variable Data if usChoice is any positive number.
// It returns 0, if it is successful.
//
CPU_INT32S ReadFromFile(char* FileName, char* Tag, char* Data1, char* Data2)
{
    CPU_INT32S nStatus;
    ...
    // Copying the desired command
    //
    sprintf(g_cCmdBuf, "cat %s %s", FileName, Tag);
    ...
    //
    // Check the status of command. If it is executed without any error, then
    // we have our data in g_cTmpBuf.
    //
    if( !nStatus )
    {
        //
        // Reading Data1 and Data2 from g_cTmpBuf without reading Tag.
        //
        sscanf(g_cTmpBuf, "%*s %s %s", Data1, Data2);

        //
        // Removing '<' and'>' from Data1 by following two lines.
        //
        sscanf(Data1, "%*c %s", Data1);
        Data1[strlen(Data1)-1] = 0;

        //
        // Removing '<' and'>' from Data2 by following two lines.
        //
        sscanf(Data2, "%*c %s", Data2);
        Data2[strlen(Data2)-1] = 0;
    }

    //
    // Otherwise copy "Not_Found" word in required fields.
    //
    else
    {
        //
        // Copy "Not_Found" in remaining field of Data.
        //
        Data1 = "Not_Found";
        Data2 = "Not_Found";
    }
...
/*
   This function initialize the Sd Card and mount the Fat filesystem.
   returns 1 if not initialized otherwise 0.
*/
CPU_INT32S SDCardInit(void)
{
    FRESULT fresult;
